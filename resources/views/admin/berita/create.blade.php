...
<head>
    ...
    <title>Tambah Berita | MTs Arridho</title>
...
<body class="min-h-screen bg-slate-100 text-slate-800">
    <header class="border-b border-slate-200 bg-white">
        <div class="mx-auto flex max-w-5xl items-center justify-between px-5 py-4 lg:px-8">
            <a href="{{ route('admin.berita.index') }}" class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="font-bold text-emerald-900">Tambah Berita</h1>
    ...
    <main class="mx-auto max-w-5xl px-5 py-10 lg:px-8">
        <form action="{{ route('admin.berita.store') }}" method="POST" enctype="multipart/form-data" class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-9">
            @csrf

            <p class="text-sm font-bold uppercase tracking-widest text-emerald-700">Berita Baru</p>
            <h2 class="mt-2 text-3xl font-bold text-slate-900">Masukkan Informasi Berita</h2>
            ...
            <div class="mt-8 space-y-6">
                <div>
                    ...
                    <input type="text" id="judul" name="judul" value="{{ old('judul') }}" required placeholder="Contoh: Kegiatan Masa Ta'aruf Siswa" class="w-full rounded-xl border border-slate-300 px-4 py-3 outline-none transition focus:border-emerald-600 focus:ring-4 focus:ring-emerald-100">
                ...
                <div>
                    ...
                    <textarea id="ringkasan" name="ringkasan" rows="3" maxlength="500" placeholder="Tuliskan ringkasan singkat berita" class="w-full rounded-xl border border-slate-300 px-4 py-3 outline-none transition focus:border-emerald-600 focus:ring-4 focus:ring-emerald-100">{{ old('ringkasan') }}</textarea>
                ...
                <div>
                    ...
                    <textarea id="isi" name="isi" rows="12" required placeholder="Tuliskan isi lengkap berita" class="w-full rounded-xl border border-slate-300 px-4 py-3 outline-none transition focus:border-emerald-600 focus:ring-4 focus:ring-emerald-100">{{ old('isi') }}</textarea>
                    @error('isi')<p class="mt-2 text-sm text-red-600">{{ $message }}</p>@enderror
                </div>

                <div>
                    <label for="gambar" class="mb-2 block text-sm font-semibold text-slate-700">Gambar berita</label>
                ...
                <div>
                    ...
                    <select id="status" name="status" required class="w-full rounded-xl border border-slate-300 bg-white px-4 py-3 outline-none transition focus:border-emerald-600 focus:ring-4 focus:ring-emerald-100">
                        <option value="draft" @selected(old('status', 'draft') === 'draft')>Simpan sebagai Draft</option>
                        <option value="published" @selected(old('status') === 'published')>Terbitkan Sekarang</option>
                    </select>
            ...
            <div class="mt-9 flex flex-col-reverse gap-3 border-t border-slate-200 pt-7 sm:flex-row sm:justify-end">
                ...
                <button type="submit" class="rounded-xl bg-emerald-700 px-7 py-3 font-semibold text-white shadow-sm transition hover:bg-emerald-800">Simpan Berita</button>
            </div>
        </form>
    </main>
</body>
